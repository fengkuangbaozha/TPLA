...
$script/modify_fastq_v1.pl -r $arg_working_dir -n $arg_input_name -a $arg_working_dir/${arg_input_name}# -s F -p 17 -l 25 -5 Uni -3 Index    ##########get the clean reads

#########################assemble the reads into transcripts####################
export LD_LIBRARY_PATH=/psc/program/install/boost-1.55/lib:$LD_LIBRARY_PATH
$tophat/tophat -p 32 -G $gff --library-type fr-firststrand -o $arg_working_dir/${arg_input_name}_top $genome $arg_working_dir/$arg_input_name/Len_sort_step_4/${arg_input_name}_1.fastq.fq.trimmed.paired1.cut.paired1 $arg_working_dir/$arg_input_name/Len_sort_step_4/${arg_input_name}_1.fastq.fq.trimmed.paired1.cut.paired2  #$file/SRR331227_1.fastq.fq.trimmed.single.cut.single1 
$cufflink/cufflinks -o $arg_working_dir/${arg_input_name}_cuff -p 32 --library-type fr-firststrand -g $gff $arg_working_dir/${arg_input_name}_top/accepted_hits.bam         #####assmeble the reads using cufflinks
