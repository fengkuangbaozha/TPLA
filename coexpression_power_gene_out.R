...
gene.high <- read.delim(file=args[1],header=T,stringsAsFactors = FALSE,row.names=1) 
...
#kmean.floral <- read.delim(file=args[4],header=F,stringsAsFactors = FALSE)
...
kmean.floral <- coexpression.power(t(lnc_gene),filename)
...
write.table(adj.test_lnc, paste(filename,"correlation.value.txt",sep=""), sep="\t", row.names=TRUE, col.names=NA,quote=FALSE)
adj.test_lnc.table <- as.data.frame(as.table(adj.test_lnc))
write.table(adj.test_lnc.table, paste(filename,"correlation.value.each.corres.txt",sep=""), sep="\t", row.names=FALSE, col.names=FALSE,quote=FALSE)
...
adj.test_lnc.top <- t(apply(adj.test_lnc,1,function(x){y <- colnames(adj.test_lnc)[order(x,decreasing = TRUE)]; y <- y[1:22]; return(y)})) ###get the top 20 large correlation value
