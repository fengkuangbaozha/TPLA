...
sed '/\t0.5\t/d' $dir/split_cuff/cuff-rfmodel_seqstruall_score.txt |cut -f 1 |sort -u > $dir/split_cuff/cuff-rfmodel_seqstruall.name1
#cut -f 1 $dir/split_cuff/cuff-rfmodel_seqstruall_score.txt |sort -u > $dir/split_cuff/cuff-rfmodel_seqstruall.name1
grep -Fvf $dir/${name}.dot.del $dir/split_cuff/cuff-rfmodel_seqstruall.name1 > $dir/split_cuff/cuff-rfmodel_seqstruall.name
perl $script/extractFromFasta.pl $dir/split_cuff/cuff-protdb_blast_left.fa list $dir/split_cuff/cuff-rfmodel_seqstruall.name > $dir/${name}-CPC_left.fa   ###get the diff sequence

########get the gtf file for lncRNA##########
grep -Fwf $dir/split_cuff/cuff-rfmodel_seqstruall.name $dir/${name}.combined.cuffnorm.gtf > $dir/${name}-CPC_left.gtf   ####generate lncRNA gtf file
...
wc $dir/split_cuff/cuff-rfmodel_seqstruall.name
...
##################identify housekeeping RNAs using blast############
bedtools intersect -a $dir/${name}-CPC_left.gtf -b $ref |awk '{        for (i = 1; i <= NF; i++) {  if ($i ~ /gene_id|transcript_id|class_code/){  printf "%s %s", $i, $(i+1)  }  }  print ""}' |sort -u |sed 's/"//g' |sed 's/;/\t/g' |awk -F" " '{OFS="\t"; print $4,$2,$6,"housekeepRNA"}' > $dir/cuff.housekeepingRNA.name
cut -f1 $dir/cuff.housekeepingRNA.name |sort -u > $dir/tmp
grep -Fvf $dir/tmp $dir/${name}-CPC_left.lncname > $dir/${name}-CPC_left.lncname.iuox.del    ####delete TE and hkRNA
grep -Fvf $dir/tmp $dir/${name}-CPC_left.gtf > $dir/${name}-CPC_left.del.gtf
cut -f2 $dir/${name}-CPC_left.lncname.iuox.del |sort -u > $dir/${name}-CPC_left.lncname.iuox.del.name
perl $script/extractFromFasta.pl $dir/${name}-CPC_left.fa list $dir/${name}-CPC_left.lncname.iuox.del.name > $dir/${name}-CPC_left.lncname.iuox.del.fa
#Rscript /psc/bioinformatics/sunyd/identify/script/lengthdistribution.R $dir/${name}-CPC_left.fa $dir/${name}-CPC_left   ###calculate length
