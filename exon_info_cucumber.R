...
gtf <- read.delim("~/sunyd/identify/cucumber_rnaseq/SRRadd/cuff75.combined.gtf-exon",header=F,stringsAsFactors = FALSE) 
colnames(gtf) <- c("gene_id","transcript_id","class_code","exonlength","start","end","strand")
seqcomp <- read.delim("~/sunyd/identify/cucumber_rnaseq/SRRadd/lnclength.txt",header=F,stringsAsFactors = FALSE,row.names=1)
...
lncname <- read.delim("~/sunyd/identify/cucumber_rnaseq/SRRadd/cuff75-CPC_left.lncnamedel-iuox",header=F,stringsAsFactors = FALSE)
...
lncbar <- barinput(lncexon,5)
lncbar <- c(lncbar,0)
lncbardel <- barinput(lncexondel,6)
...
genegccum <- cumplot(seqcomp[genename,3])
lncgccum <- cumplot(seqcomp[lncname,3])
lncgccumdel <- cumplot(seqcomp[lncnamedel,3])
...
pdf("lnc_au.pdf")
plot(genegccum[,1],genegccum[,2],type="l",col="blue",xlab="AU content",ylab="Cumulative frequency",main="The AU content of transcripts")
...
#print(nrow(genename))
#lncgenename <- nameext(lncname)  ######calculate the lncRNA info from lncRNA gtf file
