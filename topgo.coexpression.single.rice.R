...
filename <- args[2]
lncrnaInterestingGenes <- as.matrix(read.delim(file=args[1],header=F,row.names=1,stringsAsFactors=FALSE))[,1:20]
...
library(topGO)
library(ALL)
	anoGO <- readMappings("/psc/bioinformatics/sunyd/genome/oryza/OryzaMSU7.go.ano")
...
#######analyze the GO annotation result#####################
        BPtable <- GenTable(GOBP, pvalue = BPfis, topNodes = 38)
        MFtable <- GenTable(GOMF, pvalue =MFfis, topNodes = 38)
        CCtable <- GenTable(GOCC, pvalue = CCfis, topNodes = 38)
