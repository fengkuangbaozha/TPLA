...
sed -n '/\tCG/p' $dir/${name}.${location}.gtf |cut -f 9,18 |cut -d " " -f 2,3,6 |awk '{OFS="\t"; print$4,$1,$2}' |sed 's/;//g' |sed 's/"//g' |sort > $dir/${name}.${location}.cg.meth
...
sed -n '/\tCHG/p' $dir/${name}.${location}.gtf |cut -f 9,18 |cut -d " " -f 2,3,6 |awk '{OFS="\t"; print$4,$1,$2}' |sed 's/;//g' |sed 's/"//g' |sort > $dir/${name}.${location}.chg.meth
...
sed -n '/\tCHH/p' $dir/${name}.${location}.gtf |cut -f 9,18 |cut -d " " -f 2,3,6 |awk '{OFS="\t"; print$4,$1,$2}' |sed 's/;//g' |sed 's/"//g' |sort > $dir/${name}.${location}.chh.meth
